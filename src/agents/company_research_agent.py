...
# Open AI chat from haystack
from haystack.components.generators.chat import OpenAIChatGenerator
from haystack.dataclasses import ChatMessage
# this is to create an agent that can use tools to answer questions
from haystack.components.agents import Agent
#This lets the agent connect to external tools through MCP.
...
def create_company_research_agent():
    ...
    server_info = SSEServerInfo(
     ...
     ## this line means ""I don't have the search tool locally. Ask the MCP server for it.""
     ## we are only using one mcp tool here named search tool from mcp.
...
def run_company_research_agent(company_name: str) -> Dict[str, Any]:
    # output is dictionary with key=string and value=anything..
    """
