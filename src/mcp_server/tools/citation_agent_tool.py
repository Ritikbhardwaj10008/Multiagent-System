...
#ComponentTool converts any Haystack component or pipeline into a tool that an AI agent can call.
...
## now search pipeline become a tool now llm can call it.
    ...
    tools = [search_tool]
   ##************************** now create an agent in which we have llm and this tool as well*****************************
    agent = Agent(
        chat_generator = llm,
        tools = tools
    )
