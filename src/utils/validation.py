...
#******************************why we need validation.py*******************************************
#The validation.py file acts as a safety layer for the entire multi-agent pipeline. Since LLMs do not always return data in the exact format we expect, validation ensures that each agent’s input and output follow the schemas defined in models.py. It catches problems such as missing fields, incorrect data types, or invalid JSON before the data moves to the next agent, making errors easier to debug and preventing failures later in the pipeline. It also serves as documentation by clearly specifying what data each agent expects and produces. Although the current pipeline already performs some validation through Pydantic in safe_run(), having a separate validation layer is a good practice for larger or production systems where multiple developers, APIs, databases, and external inputs are involved.